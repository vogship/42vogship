...
vogship() {
	...
	if [ -n "$flags" ]; then
		if grep -qoE '[v]' <<< $flags; then
			# Print Version
			printf "\e[92mVersion v1.2.5 \e[97m(\e[92mBeta\e[97m)\e[0m\n"
			printf "\e[90mPatchVer591246\e[0m\n"
			...
			if grep -qoE '[f]' <<< $flags; then
				...
				if grep -q "vogShip v1.2.5 (BETA) pV 591246" "vogship-version-check"; then
		...
		printf "\e[96mWritten by Aaron Menadue\e[0m\n"
		printf "\e[96mContributions from:\e[0m\n"
		printf "\e[96m  Byron Hawksmith  Jaymie Gobbett\e[0m\n"
		printf "\e[96m  Pasquale Rossi   Tomas Turner  \e[0m\n"
		printf "\e[96m  Coby J. Morris                 \e[0m\n"
    fi
}
